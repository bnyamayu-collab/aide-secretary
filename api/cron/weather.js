...
// 大分県津久見市の天気が雨or雪の場合のみ通知
// 3日間予報で雨/雪がある場合も通知
// =====================================================================
import { push } from '../../lib/line.js';
import { chat } from '../../lib/gemini.js';
import { getAllUserIds, getProfile } from '../../lib/store.js';
...
async function checkAndNotifyWeather(userId) {
  ...
  // Step 1: Ask Gemini to check 3-day weather with grounding (web search)
...
weatherは「晴れ」「曇り」「雨」「雪」「雷雨」「みぞれ」等。precipitationは降水確率%。`;
  ...
  if (!todayBad && !tomorrowBad && !dayAfterBad) {
    console.log(`${location}: All clear (${forecast.today?.weather}, ${forecast.tomorrow?.weather}, ${forecast.day_after?.weather}), no notification`);
    return; // 晴れ or 曇り only → no notification
  }
  ...
  // Add advice if today is bad weather
  if (todayBad) {
    msg += '\n\n💡 ';
    ...
    // Today is fine but future days have rain/snow
    const futureBadDays = [];
